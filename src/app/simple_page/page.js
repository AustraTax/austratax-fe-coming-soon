...
export default function SimplePage() {
  const [state, setState] = useState({
    income: '',   taxPaid: '',
    deductions: [], newCategory: 'Work from home', newAmount: '',
    year: 'this', taxFreeFlag: 'yes',
    hasZoneOffset: 'no', zoneOffset: '', zoneDays: '',
    useKmVehicle: 'yes', vehicleKm: '', vehicleRate: '0.78', vehicleManual: '',
    remoteOffsetFlag: 'no', helpFlag: 'no', medicareExemptFlag: 'no',
    privateHealthFlag: 'yes', hasDependants: 'no', numDependants: ''
  });
  ...
  const handlers = {
    ...
    removeDeduction:      idx => setState(s => ({ ...s, deductions: s.deductions.filter((_,i)=>i!==idx) })),
    ...
    setRemoteOffsetFlag:  val => setState(s => ({ ...s, remoteOffsetFlag: val })),
    setHelpFlag:          val => setState(s => ({ ...s, helpFlag: val })),
    setMedicareExemptFlag:val => setState(s => ({ ...s, medicareExemptFlag: val })),
    setPrivateHealthFlag: val => setState(s => ({ ...s, privateHealthFlag: val })),
  ...
  const [summary, setSummary] = useState({
    tax:0, taxableBefore:0, taxable:0,
    lito:0, helpRepayment:0,
    medicareLevy:0, medicareSurcharge:0,
    marginalRate:0, netIncome:0, refundMessage:''
  });
  ...
  useEffect(() => {
    ...
    const zoneVal = s.hasZoneOffset==='yes'?(rates[s.zoneOffset]||0)*days/365:0;
    const vehDed = s.useKmVehicle==='yes'?
    ...
    else if(afterThreshold<=120000) bracketTax=7125 + (afterThreshold-37500)*0.325;
    else if(afterThreshold<=180000) bracketTax=29467 + (afterThreshold-120000)*0.37;
    else bracketTax=51667 + (afterThreshold-180000)*0.45;

    let lOff=0;
    if(afterThreshold<=37500) lOff=700;
    else if(afterThreshold<=66667) lOff=Math.max(0,700-(afterThreshold-37500)*0.05);

    const helpRep = s.helpFlag==='yes'?calculateHelpRepayment(afterThreshold):0;
    const med     = s.medicareExemptFlag==='yes'?0:baseTaxable*0.02;
    const surcharge = s.privateHealthFlag==='no'&&baseTaxable>140000?baseTaxable*0.01:0;
  ...
  return (
    <main className="min-h-screen p-8 bg-gray-50">
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
        <TaxForm state={state} handlers={handlers} addDeduction={addDeduction} />
