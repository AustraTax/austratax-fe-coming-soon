// File: src/app/australian-tax-returns-calculator/page.js
'use client';
...
export default function SimplePage() {
  ...
  const handlers = {
    ...
    removeDeduction:       i => setState(s => ({ ...s, deductions: s.deductions.filter((_, j) => j !== i) })),
    setYear:               v => setState(s => ({ ...s, year: v })),
    setIsResident:         v => setState(s => ({ ...s, isResident: v })),
    setVisaStatus:         v => setState(s => {
    ...
    setRemoteOffsetFlag:   v => setState(s => ({ ...s, remoteOffsetFlag: v })),
    setHelpFlag:           v => setState(s => ({ ...s, helpFlag: v })),
    setMedicareExemptFlag: v => setState(s => ({ ...s, medicareExemptFlag: v })),
    setPrivateHealthFlag:  v => setState(s => ({ ...s, privateHealthFlag: v })),
  ...
  useEffect(() => {
    ...
    const zoneD     = s.zoneOffset ? (zoneRates[s.zoneOffset]||0)*days/365 : 0;

    // Vehicle deduction
    const vehD = s.useKmVehicle === 'yes'
    ...
    const cat     = s.isResident==='yes'
                    ? 'resident'
                    : visaRec?.residency || 'nonResident';
    ...
    const lito = (cat==='resident' && cfg.lito)
      ? (() => {
          ...
          return Math.max(0, maxOffset - (afterThreshold - phaseOutStart)*rate);
        })()
      : 0;

    // HELP repayment
    const helpRep = s.helpFlag==='yes'
      ? calculateHelpRepayment(afterThreshold)
      : 0;
    ...
    const surcharge = (s.privateHealthFlag==='no' && taxableBefore>90000)
      ? taxableBefore * 0.01
      : 0;

    // Total tax & net income
    const totalTax = Math.max(0, bracketTax - lito) + helpRep + medicare + surcharge;
    const netIncome = gross - totalTax;

    // Marginal rate (last bracket)
    const lastB = cfg.brackets.slice().reverse()
                      .find(b=>taxableBefore>=b.min) || cfg.brackets[0];
    const marginalRate = lastB.rate * 100;

    // Refund or owing
    const refundMessage =
      paid > totalTax ? `🟢 Refund: $${(paid - totalTax).toFixed(2)}`
      : paid < totalTax ? `🔴 Owe:   $${(totalTax - paid).toFixed(2)}`
      :                   '✅ No balance owing';
